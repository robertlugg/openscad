...
Value builtin_dxf_dim(const std::shared_ptr<Context> ctx, const std::shared_ptr<EvalContext> evalctx)
{
	...
	for (size_t i = 0; i < dxf.dims.size(); i++)
	{
		...
		if (type == 0) {
			...
			return dxf_dim_cache.emplace(key, Value(distance_projected_on_line)).first->second.clone();
		}
		else if (type == 1) {
			// Aligned
			double x = d->coords[4][0] - d->coords[3][0];
			double y = d->coords[4][1] - d->coords[3][1];
			return dxf_dim_cache.emplace(key, Value(sqrt(x*x + y*y))).first->second.clone();
		}
		else if (type == 2) {
			// Angular
			...
			return dxf_dim_cache.emplace(key, Value(std::fabs(a1 - a2))).first->second.clone();
		}
			...
			// Ordinate
			return dxf_dim_cache.emplace(key, Value((d->type & 64) ? d->coords[3][0] : d->coords[3][1])).first->second.clone();
		}
